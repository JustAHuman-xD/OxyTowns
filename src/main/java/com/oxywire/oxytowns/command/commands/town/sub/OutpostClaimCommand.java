...
public final class OutpostClaimCommand {
    ...
    @CommandMethod("town|t outpost claim confirm")
    @CommandDescription("Confirm claiming an outpost for your town")
    @AcceptConfirmation("player_claim_outpost")
    @Hidden
    @MustBeInTown
    public void onOutpostClaimConfirm(final Player sender, final @SendersTown Town town) {
    ...
    @CreateConfirmation("player_claim_outpost")
    @MustBeInTown
    public void onOutpostClaim(final Player sender, final @SendersTown Town town) {
        final Messages messages = Messages.get();
        final Config config = Config.get();
        final int outpostPrice = (int) config.getOutpostPrice();
        ...
        messages.getTown().getOutpost().getClaimConfirm().send(sender,
            Formatter.number("price", outpostPrice),
            Placeholder.unparsed("town", town.getName()));
    }
}
